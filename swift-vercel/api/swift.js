// api/swift.js — Vercel Serverless Function (v4: Inode & Buffer optimized)
// Downloads Swift toolchain to /tmp, using synchronous writes to avoid buffer overflows.
...
// Radical skip list to minimize Inodes and Disk Space
const SKIP_PREFIXES = [
  "usr/bin/swift-", "usr/bin/sourcekit", "usr/bin/lldb", "usr/bin/llvm-",
  "usr/lib/swift/pm/", "usr/lib/swift_static/", "usr/lib/sourcekitd",
  "usr/lib/libsourcekitd", "usr/lib/libIndexStore", "usr/share/",
  "usr/lib/swift/clang/lib/", "usr/lib/swift/iphone", "usr/lib/swift/macosx",
  "usr/lib/swift/appletv", "usr/lib/swift/watch", "usr/lib/swift/windows",
  "usr/lib/swift/linux/x86_64", // We keep what's strictly needed
  "usr/lib/swift/host", "usr/lib/swift/shims", // Dangerous to skip shims, but we might skip others
];

function shouldSkip(relPath) {
  // Always keep essential binaries
  if (relPath === "usr/bin/swiftc" || relPath === "usr/bin/swift") return false;
    ...
    function processBuffer() {
      while (true) {
        ...
        if (remaining > 0) {
          ...
          if (currentFd !== null) {
            try { fs.writeSync(currentFd, chunk); } catch (e) {
               if (e.code === 'ENOSPC') {
                 log("ENOSPC during writeSync");
                 throw e; 
               }
            }
          }
