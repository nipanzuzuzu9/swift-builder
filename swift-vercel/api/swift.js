...
// that are never needed for compile+run: package manager, IDE tools, docs,
// and stdlib copies for non-Linux platforms.
const SKIP_PREFIXES = [
...
async function streamExtractTarGz(urlStr, destDir, stripComponents = 1) {
  mkdirpSync(destDir);
  log(`Streaming download → gunzip → selective tar extract into ${destDir}`);
    ...
    function processBuffer() {
      while (true) {
        if (!header) {
          ...
          // Skip bloat paths — saves ~220MB, keeping /tmp usage under 160MB
          if (shouldSkip(relPath)) {
            // Must still consume the data bytes or the stream gets out of sync
            header._skip = true;
          }

          if (header.typeflag === "5" || (header.size === 0 && header.name.endsWith("/"))) {
            // Directory — only create if not skipped
          ...
          if (header.typeflag === "2" || header.typeflag === "K") {
            ...
            // Regular file — only open a write stream if not in skip list
            if (!header._skip) {
              ...
              try {
                ...
                outStream.on("error", (err) => {
                  if (err.code === 'ENOSPC') {
                    log("CRITICAL: No space left on device during extraction!");
                  }
                  reject(err);
                });
              } catch (e) {
...
async function downloadSwift() {
  ...
  // Aggressive cleanup before download
  cleanup(true);

  // Check available /tmp space
  // Selective extraction needs ~160MB. We check for 200MB to be safe.
  ...
  if (!isNaN(freeMB) && freeMB < 200) {
    // If still not enough space, try one more cleanup and re-check
    cleanup(true);
    const freeMB2 = parseInt(sh("df -BM /tmp | tail -1 | awk '{print $4}'").stdout.trim().replace("M", ""), 10);
    if (freeMB2 < 200) {
      throw new Error(`/tmp has only ${freeMB2}MB free — need ~200MB. Vercel /tmp might be full from other processes.`);
    }
  }
...
async function runSwift(code, action = "run", args = []) {
  ...
  // Cleanup old workdirs if we are low on space
  cleanup();
  ...
  if (action === "run") {
    ...
    // Immediate cleanup of stderr file
  ...
  try {
    ...
    for (const e of fs.readdirSync("/tmp")) {
      ...
      try {
        ...
        // Normal cleanup: 5 minutes old. Aggressive: 0 minutes old (everything not current).
        const maxAge = aggressive ? 0 : 300_000; 
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // If we hit ENOSPC, try aggressive cleanup for the next run
    if (err.message.includes("ENOSPC") || err.code === 'ENOSPC') {
      cleanup(true);
    }
