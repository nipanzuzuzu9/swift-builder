...
// v5: Radical Selective Extraction. We ONLY keep what's strictly required for "Hello World".
function shouldKeep(relPath) {
  // 1. Essential binaries
  if (relPath === "usr/bin/swiftc") return true;
  if (relPath === "usr/bin/swift") return true;
  if (relPath === "usr/bin/swift-frontend") return true;

  // 2. Essential core libs (Glibc, CoreFoundation, etc.)
  if (relPath.startsWith("usr/lib/swift/linux/x86_64/")) {
    // Skip static libs, only keep .swiftmodule and .swiftdoc if needed, but primarily .so
    if (relPath.endsWith(".a")) return false;
    return true;
  }
  
  // 3. Essential shims & headers
  if (relPath.startsWith("usr/lib/swift/shims/")) return true;
  if (relPath.startsWith("usr/lib/swift/linux/Glibc.swiftmodule/")) return true;

  // 4. Clang headers (needed for Glibc import)
  if (relPath.startsWith("usr/lib/swift/clang/include/")) return true;
        ...
        if (remaining > 0) {
          ...
          if (currentFd !== null) {
            try { 
              ...
              if (totalWritten > 100 * 1024 * 1024) {
                 throw new Error("Vercel Write Quota (100MB) reached. Aborting extraction.");
              }
            } catch (e) { throw e; }
          }
