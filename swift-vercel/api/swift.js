...
const SKIP_PREFIXES = [
  ...
  "usr/lib/swift/linux/x86_64", // We keep usr/lib/swift/linux but might skip static libs if needed
];
  ...
  await new Promise((resolve, reject) => {
    ...
    function checkSpace() {
      ...
      if (now - lastCheck > 2000) { // Check every 2 seconds
        lastCheck = now;
        const free = parseInt(sh("df -BM /tmp | tail -1 | awk '{print $4}'").stdout.trim().replace("M", ""), 10);
        if (free < 20) { // Critical threshold
          log(`CRITICAL SPACE: ${free}MB left. Attempting emergency cleanup.`);
          cleanup(true);
        }
      }
    }
...
async function downloadSwift() {
  ...
  // Use a smaller initial threshold to allow for shared environments
  const freeMB = parseInt(sh("df -BM /tmp | tail -1 | awk '{print $4}'").stdout.trim().replace("M", ""), 10);
  log(`/tmp free: ${freeMB}MB`);
  
  // Even if 0MB is reported, we try cleanup first as it might be a stale reporting or just 1-2 files blocking
  if (freeMB < 180) {
    cleanup(true);
    const freeMB2 = parseInt(sh("df -BM /tmp | tail -1 | awk '{print $4}'").stdout.trim().replace("M", ""), 10);
    if (freeMB2 < 180) {
       // We still try to proceed but log a warning, as Vercel sometimes reports 0 when it's actually just very low
       log(`WARNING: Low space (${freeMB2}MB). Proceeding with caution.`);
    }
  }
  ...
  try {
    ...
    // If we fail, wipe the partial install to free space
    sh(`rm -rf "${SWIFT_DIR}"`);
    throw e;
  }
...
async function runSwift(code, action = "run", args = []) {
  ...
  if (action === "run") {
    ...
    // Clean up binary and source immediately
    ...
    for (const e of files) {
      if (e.startsWith("sw-") || e.startsWith("main") || e.endsWith(".txt")) {
        ...
        try {
          ...
          if (aggressive || (now - stats.mtimeMs > 60_000)) { // 1 minute for normal cleanup
            sh(`rm -rf '${full}'`);
          }
        } catch (_) {}
      }
